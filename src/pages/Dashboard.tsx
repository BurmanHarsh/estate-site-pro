  ...
  useEffect(() => {
    const checkAuthAndFetchQuota = async () => {
      ...
      try {
        ...
        const uploadsRes = await fetch("http://localhost:5000/api/users/my-uploads", {
          headers: { Authorization: `Bearer ${jwtToken}` },
        });
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      if (remainingQuota >= count) {
        ...
        let price = 0;
        if (paidPhotosCount === 1) price = 5;
        else if (paidPhotosCount === 2) price = 9;
        else if (paidPhotosCount >= 5)
          price = 20; // Example flat tier, can adjust per instructions
        else price = paidPhotosCount * 5; // Fallback
  ...
  const simulateUpload = async () => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:5000/api/uploads", {
        ...
        body: JSON.stringify({ filePaths, tier: "Premium", instructions }),
      });
      ...
      if (res.ok) {
        ...
        // Refresh uploads
        fetch("http://localhost:5000/api/users/my-uploads", {
                    ...
                    <div className="bg-black/80 border border-white/10 px-6 py-5 rounded-none w-full max-w-sm text-left">
                      ...
                      {calculatedPrice > 0 && (
                        <p className="text-[10px] uppercase tracking-widest text-white/30 mt-4 leading-relaxed font-light">
                          *Free tier exhausted. Rates: 1 photo = $5 | 2 photos = $9 | 5 photos = $20.
                        </p>
                      )}
                    </div>
              ...
              <div className="space-y-8 animate-fade-in">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-px bg-white/10 border border-white/10">
                    <button className="bg-white text-black p-6 text-left relative">
                      <p className="text-[10px] tracking-widest uppercase mb-2 opacity-60">Foundation</p>
                      <p className="font-display text-xl mb-1">$2<span className="text-sm font-sans tracking-normal opacity-60">/photo</span></p>
                      <p className="text-xs opacity-70">
                        48-hour delivery
                      </p>
                    </button>
                    <button className="bg-black p-6 text-left hover:bg-zinc-900 transition-colors relative border border-transparent">
                      <p className="text-[10px] tracking-widest uppercase mb-2 text-white/50">Signature</p>
                      <p className="font-display text-xl mb-1 text-white">$5<span className="text-sm font-sans tracking-normal text-white/50">/photo</span></p>
                ...
                <Button
                  className="w-full bg-white/5 text-white/30 border border-white/10 rounded-none hover:bg-white/5 transition-colors uppercase tracking-widest text-[10px] py-6 shadow-none mt-2"
                  disabled
                >
                  Proceed to Checkout
                </Button>
                <p className="text-[10px] uppercase tracking-widest text-white/30 text-center mt-4">
